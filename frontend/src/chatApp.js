...
function setSyncPlayerVisible(visible) {
  ...
  if (chatEl) {
    chatEl.classList.toggle("watch-mode", Boolean(visible));
  }
}
...
function loadSyncedVideo(sourceUrl, sourceTitle, sourceKind = "") {
  ...
  if (!streamInfo) {
    showToast("Unsupported stream URL. Try YouTube, Google Drive, Dropbox, Spotify, Google Photos, or a direct video/audio file.", { type: "error" });
    return;
  }
...
function startStreamFromInput() {
  ...
  try {
    ...
    title = `Shared from ${parsed.hostname.replace(/^www\\./i, "")}`;
  } catch {
    // Keep fallback title.
  }
